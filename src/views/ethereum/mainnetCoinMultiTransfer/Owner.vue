...
export default {
  ...
  setup() {
    ...
    const { getMultiOwner, getContractList, contractList } = tokenListData();
    //토큰 리스트 호출
    getContractList(0);
    const getOwnerAddress = async () => {
      ...
      response.value = await getMultiOwner(contractSelected.value);
